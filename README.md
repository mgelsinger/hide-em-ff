...
- Not a moderation tool, recommendation engine, or content classifier. It is a tool for one user, doing exactly what they configured.
...
- **AMO:** submission in flight; the listing is not live yet. Use the build-from-source path below until then.
...
3. **Grant host permissions:** click the puzzle icon in the toolbar, find hide-em, then choose **Always Allow on All Sites** (or per-site as you visit them). Firefox MV3 makes `<all_urls>` opt-in, so the content script won't run until you do this.
...
It also self-terminates if any single drain exceeds 1s or cumulative scan time exceeds 60s, surfacing a structured `console.warn` with the relevant counters. This is a deliberate failure mode: a visible flash beats a blank page.
...
The engine has full coverage (rules, scopes, normalization edge cases including ZWJ, diacritics, fullwidth Unicode). The scanner is not unit-tested; it is DOM-coupled and JSDOM tests of `MutationObserver` behavior have low ROI. Verification is the kill-switch safety net plus the debug overlay plus manual smoke testing.
